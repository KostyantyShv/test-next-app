...
>(
  (
    ...
    const baseStyles =
      "w-full p-2.5 border border-border-color rounded text-text-color focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors";
    // Якщо className містить bg-*, не додаємо !bg-white
    const hasBgClass = className && /bg-\[|bg-/.test(className);
    const defaultBg = hasBgClass ? "" : "!bg-white";
    const readOnlyStyles =
      readOnly || disabled
        ? (hasBgClass ? "" : "!bg-white") + " cursor-not-allowed opacity-80"
        : "";
    const inputStyles = `${baseStyles} ${defaultBg} ${readOnlyStyles} ${className}`;
