...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  const handleEventClick = (event: Event, e: React.MouseEvent) => {
    // Don't expand if clicking on action buttons or time pill
    if ((e.target as HTMLElement).closest('.event-action-btn')) return;
    if ((e.target as HTMLElement).closest('.event-time')) return;
    ...
    if (onEventExpand && cellIndex !== undefined) {
      ...
      if (event.id) {
        ...
        eventIdentifier = `${cellIndex}-${event.title}`;
      }
  ...
  return (
    ...
    >
      <div
        className={`date-number text-sm font-medium mb-1 ${isPrevMonth || isNextMonth ? "text-[var(--subtle-text)] is-muted" : "text-[var(--bold-text)]"
          }`}
      >
        {date}
      </div>
      {displayEvents.map((event, index) => {
        ...
        const eventColor = eventColorMap[event.type || ""] || "";
        const isTimeHovered = timeHoveredId === eventIdentifier;

        const avatars = event.participants?.slice(0, 2) || [];
        const displayCount = event.attendeeCount || (event.attendees > 2 ? `${event.attendees}+` : `${event.attendees}`);

        return (
          <div
            key={event.id || index}
            className={`event group w-full rounded-[4px] cursor-pointer transition-colors mb-0.5 relative ${isExpanded
                ? "expanded p-2 bg-[var(--surface-secondary)]"
                : "px-2 py-[2px] bg-[var(--gray-100)] hover:bg-[var(--hover-bg)]"
          ...
          >
            <div className="event-header flex items-center gap-1">
              <div className="event-title-row flex items-center flex-1 min-w-0 relative h-5">
                <div
                  ...
                  style={eventColor ? { color: eventColor } : undefined}
                >
                  {event.title}
                </div>
                {!isTimeHovered && (
                  <div
                    className={`event-actions absolute right-0 top-0 bottom-0 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity flex items-center gap-1 pl-2 ${isExpanded ? "bg-[var(--surface-secondary)]" : "bg-[var(--hover-bg)]"
                      }`}
                    style={{
                      maskImage: "linear-gradient(to right, transparent, black 15px)",
                      WebkitMaskImage: "linear-gradient(to right, transparent, black 15px)",
                    }}
                  >
                    {event.listingId && (
                      <button
                      ...
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="w-3.5 h-3.5" style={{ color: "#9AA0A6" }}>
                    ...
                    >
                      <svg fill="none" viewBox="0 0 20 20" className="w-3.5 h-3.5" style={{ color: "#9AA0A6" }}>
            ...
            >
              ...
              <div className="pill-external-icon w-3 h-3 opacity-0 group-hover/time:opacity-100 transition-opacity flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 26 26" style={{ color: "#9AA0A6" }}>
                ...
                {avatars.length > 0 ? (
                  avatars.map((p, i) => (
                    <Image
                      ...
                      width={24}
                      src={p.avatarUrl}
                      className={`avatar w-6 h-6 rounded-full border-2 border-[var(--surface-color)] object-cover ${i > 0 ? '-ml-2' : ''}`}
                  ...
                  <>
                    <Image height={24} width={24} src={event.avatar1} className="avatar w-6 h-6 rounded-full border-2 border-[var(--surface-color)] -ml-2 first:ml-0" alt="Attendee 1" />
                    <Image height={24} width={24} src={event.avatar2} className="avatar w-6 h-6 rounded-full border-2 border-[var(--surface-color)] -ml-2" alt="Attendee 2" />
                  </>
                )}
              </div>
              <span className="attendee-count text-xs text-[var(--subtle-text)] bg-[var(--surface-secondary)] rounded-xl" style={{ padding: "2px 6px" }}>
                {displayCount}
              </span>
            </div>
          </div>
      ...
      {hasMoreEvents && !showMore && (
        <button
          className="more-events absolute bottom-2 left-2 px-2 py-0.5 bg-[var(--surface-secondary)] border border-[var(--border-color)] rounded text-xs text-[var(--subtle-text)] cursor-pointer hover:bg-[var(--hover-bg)] z-10"
          onClick={(e) => {
            e.stopPropagation();
            toggleMoreEvents();
          }}
