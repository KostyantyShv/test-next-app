  ...
  return (
    ...
    >
      ...
      {events?.slice(0, 3).map((event, index) => (
        ...
        >
          <div className="flex items-center gap-1">
            <div
              className={`text-xs font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[calc(100%-80px)] ${
      ...
      {events && events.length > 3 && (
        <button
          className={`absolute bottom-2 left-2 px-2 py-0.5 bg-[#F8F9FA] border border-[#E0E0E0] rounded text-xs text-[#5F6368] cursor-pointer mt-1 ${
      ...
      {events && events.length > 3 && showMore && (
        <button
          className="absolute bottom-2 left-2 px-2 py-0.5 bg-[#F8F9FA] border border-[#E0E0E0] rounded text-xs text-[#5F6368] cursor-pointer mt-1 hover:bg-[#F1F3F4]"
          onClick={toggleMoreEvents}
        >
          Show Less
        </button>
