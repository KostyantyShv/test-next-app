...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  return (
    <div
      ref={cellRef}
      className={`calendar-cell flex flex-col min-h-[120px] p-2 gap-0.5 relative bg-[var(--surface-color)] ${
        isToday ? "border-2 border-[var(--active-green)]" : ""
      }`}
    ...
    >
      <div
        ...
        className={`date-number text-sm font-medium mb-1 cursor-pointer ${
          isPrevMonth || isNextMonth ? "text-[var(--subtle-text)] is-muted" : "text-[var(--bold-text)]"
        } ${!isPrevMonth && !isNextMonth ? "hover:bg-[var(--hover-bg)] rounded" : ""}`}
      >
        {date}
      </div>
      {events?.slice(0, showMore ? events.length : 3).map((event, index) => {
        ...
        >
            ...
            <div className="event-title-row flex items-center flex-1 min-w-0 relative h-5">
              ...
              <div
                className={`event-actions absolute right-0 top-0 bottom-0 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity flex items-center gap-1 pl-2 ${
                  isExpanded ? "bg-[var(--surface-secondary)]" : "bg-[var(--hover-bg)]"
                }`}
              >
                <button
          ...
          <div className="event-time group/time inline-flex items-center gap-1 text-[12px] text-[var(--subtle-text)] px-1.5 py-[3px] rounded-full -ml-1 hover:bg-[var(--hover-bg)] cursor-pointer">
          ...
          <div className={`event-attendees items-center gap-1 mt-2 ${isExpanded ? "flex" : "hidden"}`}>
            <div className="avatar-group flex" style={{ marginRight: "4px" }}>
              ...
              <span
                className="attendee-count text-xs text-[var(--subtle-text)] bg-[var(--surface-secondary)] rounded-xl"
