...
export const ListItem: React.FC<ListItemProps> = ({
  ...
  return (
    <div
      className="calendar-list-item flex py-6 border-b border-[var(--border-color)] items-start"
    ...
    >
      ...
      >
        <div
          className={`list-weekday ${isCurrent ? "text-[var(--active-green)]" : "text-[var(--subtle-text)]"
            }`}
        ...
        <div
          className={`list-day ${isCurrent ? "text-[var(--active-green)]" : "text-[var(--text-default)]"
            }`}
      ...
      <div className="list-content flex-1 pl-0 flex flex-col gap-3">
        {events.map((event, index) => {
          ...
          return (
            <div key={event.id || index} className={`list-event-item flex items-center gap-4 py-2 ${event.type} group`}>
              <div
                className="list-event-content flex-1 min-w-0"
                style={{
                  borderLeft: `3px solid ${getEventColor(event.type)}`,
                  paddingLeft: "12px",
                }}
              >
                <div className="list-event-header flex items-center gap-2 mb-1">
                  <div
                    className="list-event-title text-base font-medium truncate"
                    style={{ color: getEventColor(event.type) }}
                  >
                    {event.title}
                  </div>
                  <div className="list-event-actions flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      className="event-action-btn w-[18px] h-[18px] flex items-center justify-center rounded-[3px] hover:bg-[var(--hover-bg)]"
                      aria-label="View listing"
                      onClick={(e) => handleViewListing(event, e)}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" style={{ color: "#9AA0A6" }}>
                    ...
                    >
                      <svg fill="none" viewBox="0 0 20 20" style={{ color: "#9AA0A6" }}>
                ...
                <div className="list-event-meta flex items-center gap-2 text-sm text-[var(--subtle-text)]">
                  <div
                    className="list-event-time-pill inline-flex items-center gap-1 px-2 py-1 rounded-full hover:bg-[var(--hover-bg)] transition-colors cursor-pointer"
                  ...
                  >
                    <div className="event-icon flex items-center justify-center" style={{ width: "16px", height: "16px" }}>
                      {event.type === "zoom-meeting" && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 38 38" className="text-[var(--event-zoom)]" style={{ width: "20px", height: "20px" }}>
                      ...
                      {event.type === "zoom-webinar" && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 38 38" className="text-[var(--event-zoom-webinar)]" style={{ width: "20px", height: "20px" }}>
                      ...
                      {event.type === "teams-meeting" && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 55 55" className="text-[var(--event-teams)]" style={{ width: "20px", height: "20px" }}>
                      ...
                      {event.type === "one-on-one" && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 38 38" className="text-[var(--event-1on1)]" style={{ width: "20px", height: "20px" }}>
                      ...
                      {event.type === "webex-meeting" && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 38 38" className="text-[var(--event-webex)]" style={{ width: "20px", height: "20px" }}>
                          <path strokeLinejoin="round" strokeLinecap="round" strokeWidth="1.2" stroke="currentColor" d="M19.8332 15.6667V14.2867C19.8332 13.9896 19.7152 13.7047 19.5051 13.4947C19.2951 13.2847 19.0102 13.1667 18.7132 13.1667H14.4132M10.6665 15.6667H22.8865V24H10.6665V15.6667ZM22.8865 17.9467L27.3332 15.2533V24.42L22.8865 21.72V17.9467Z"></path>
                        </svg>
                      )}
                      {event.type === "group-session" && (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 38 38" className="text-[var(--event-group)]" style={{ width: "20px", height: "20px" }}>
                    ...
                    <div className="pill-external-icon w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                ...
                <div className="avatar-group flex mr-1">
                  {avatars.length > 0 ? (
                    ...
                    <>
                      <Image width={24} height={24} src={event.avatar1} className="avatar w-6 h-6 rounded-full border-2 border-[var(--surface-color)] -ml-2 first:ml-0" alt="Attendee 1" />
                      <Image width={24} height={24} src={event.avatar2} className="avatar w-6 h-6 rounded-full border-2 border-[var(--surface-color)] -ml-2" alt="Attendee 2" />
                    </>
                  )}
                </div>
                <span className="attendee-count text-xs text-[var(--subtle-text)] px-1.5 py-0.5 bg-[var(--surface-secondary)] rounded-xl">
                  {displayCount}
                </span>
              </div>
              <Image
                ...
                width={120}
                src={imageUrl}
                className="list-event-image rounded-[12px] object-cover ml-auto"
