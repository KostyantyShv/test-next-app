...
const Footer: React.FC<FooterProps> = ({ isFooterVisible, images }) => {
  return (
    <div className="absolute top-0 left-0 right-0 pointer-events-none z-[20]">
      <div
        className={`fixed bottom-0 left-0 right-0 bg-white shadow-[0_-2px_8px_rgba(0,0,0,0.1)] p-2.5 flex justify-between items-center transition-transform duration-300 ease-in-out transform ${
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            <div className="flex items-center">
              ...
              <svg className="w-4 h-4 fill-[#1D77BD] ml-1" viewBox="0 0 30 30">
            ...
            <div className="flex items-center gap-2.5 mt-1">
              <span className="bg-[#00DF8B] text-[#016853] py-0.5 px-1.5 rounded text-[10px] font-semibold">
                #1 in Arts
              </span>
              <div className="flex items-center gap-0.5 text-xs">
                <span className="font-semibold text-[#089E68]">4.8</span>
                <span className="text-[#089E68]">★</span>
                <span className="text-[#5F5F5F] text-[11px]">(73)</span>
              </div>
            </div>
          </div>
        </div>
        <div className="flex gap-3">
          <button className="w-9 h-9 rounded-full flex items-center justify-center bg-black/5 hover:bg-black/10">
            <svg
