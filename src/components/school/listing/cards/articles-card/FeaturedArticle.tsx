...
export default function FeaturedArticle({ article }: FeaturedArticleProps) {
  return (
    <div className="featured-article mb-8 pb-6 border-b border-[#E0E0E0]">
      <div className="featured-content flex flex-row gap-6 max-md:flex-col">
        <div className="cover-image-container relative w-[400px] h-[280px] rounded-lg overflow-hidden flex-shrink-0 max-md:w-full max-md:h-[250px]">
          <img
            src={article.coverImage}
            alt="Featured article image"
            className="cover-image w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
          />
        </div>
        <div className="featured-details flex-1 flex flex-col">
          <span className="category text-[#016853] text-sm font-medium mb-3 uppercase tracking-[0.5px]">
            {article.category}
          </span>
          <h3 className="article-title text-[#464646] text-[22px] font-semibold mb-3 leading-[1.4] line-clamp-2 overflow-hidden text-ellipsis">
            {article.title}
          </h3>
          <p className="article-excerpt text-[#4A4A4A] text-[15px] leading-[1.6] mb-5 line-clamp-4 overflow-hidden flex-grow">
            {article.excerpt}
          </p>
