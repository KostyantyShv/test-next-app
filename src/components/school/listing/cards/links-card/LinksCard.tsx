...
const StatDot: React.FC<StatDotProps> = ({ value, type, tooltip }) => {
  return (
    <div className="relative flex items-center gap-1 md:gap-1.5 cursor-help group">
      <span
      ...
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 bg-[#333] text-white py-1 md:py-1.5 px-2 md:px-3 rounded-md text-[11px] md:text-xs whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:-translate-y-1 pointer-events-none transition-all duration-200 mb-2 z-10">
        {tooltip}
        <span className="absolute top-full left-1/2 -translate-x-1/2 border-l-[5px] border-r-[5px] border-t-[5px] border-l-transparent border-r-transparent border-t-[#333]"></span>
      </div>
    </div>
  );
};
