...
const EventCard: React.FC<EventCardProps> = ({
  ...
  // Compute displayed attendees & count based on join state
  const displayedAttendees = isInCalendar
    ? [{ image: currentUserAvatar, alt: currentUserName }, ...event.attendees]
    : event.attendees;
  const displayedCount = isInCalendar
  ...
  const titleAndAddButton = (version: "mobile" | "desktop") => {
    return (
      <div
        className={`${version === "mobile" ? "flex md:hidden" : "hidden md:flex"
        ...
        <div className="flex-1 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap mb-2 md:mb-3">
            <div className="flex items-center gap-1.5 text-[#4A4A4A] text-sm">
              ...
              <span className="relative group/date cursor-default">
                {event.date}
                {event.fullDate && (
                  <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2.5 py-1 rounded-md bg-[#262B3D] text-white text-xs font-medium whitespace-nowrap opacity-0 group-hover/date:opacity-100 transition-opacity duration-150 z-10 shadow-[0_2px_8px_rgba(0,0,0,0.15)]">
                    {event.fullDate}
                    <span className="absolute top-full left-1/2 -translate-x-1/2 border-[4px] border-transparent border-t-[#262B3D]" />
                  </span>
                )}
              </span>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center">
              {displayedAttendees.slice(0, 3).map((attendee, index) => (
            ...
            <span className="text-[#5F5F5F] text-[13px]">
              {displayedCount} participant{displayedCount !== 1 ? "s" : ""}
            </span>
          </div>
        </div>
      </div>
