...
const MobileRankCard: React.FC<MobileRankCardProps> = ({
  icon,
  title,
  value,
  trend,
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.05)] flex items-start relative transition-all duration-200 ease-in-out hover:-translate-y-[2px] hover:shadow-[0_4px_12px_rgba(0,0,0,0.1)]">
      <div className="bg-gradient-to-b from-[rgba(1,104,83,0.05)] to-[rgba(1,104,83,0.02)] p-4 flex justify-center items-center w-[100px] h-[100px] rounded-l-xl flex-shrink-0">
        {icon}
      </div>
      <div className="flex-1 p-4 flex flex-col justify-center">
        <div className="text-[#346DC2] text-[15px] font-semibold leading-[1.4] mb-2 pr-12">
          {title}
        </div>
        <div className="text-[#5F5F5F] text-sm">
          <strong className="text-[#464646] font-bold">
            {value.split(" ")[0]}
          </strong>{" "}
          {value.split(" ").slice(1).join(" ")}
        </div>
      </div>
      <div className="absolute top-4 right-4 flex items-center gap-1 p-1">
        <span
          className={`text-base font-semibold ${
            trend.direction === "up" ? "text-[#00DF8B]" : "text-[#FF4D4D]"
          }`}
        >
          {trend.value}
        </span>
...
const RankingModalMobile: React.FC<MobileDrawerProps> = ({
  isOpen,
  onClose,
}) => {
      ...
      >
        ...
        <div className="sticky top-0 bg-white z-10">
          <div className="p-5 border-b border-black/8 relative">
            ...
            <p className="text-[#5F5F5F] text-sm leading-6 m-0">
              SchoolScout rankings are based on rigorous analysis of key
              statistics from the U.S. Department of Education and millions of
              reviews.
            </p>
            <button
              className="absolute top-4 right-4 bg-none border-none text-[#5F5F5F] cursor-pointer p-2 rounded-full flex items-center justify-center transition-all duration-200 ease-in-out hover:bg-black/5"
              onClick={onClose}
            >
              <svg
                viewBox="0 0 24 24"
        ...
        <div className="flex-1 overflow-y-auto py-4 px-0">
          <h2 className="text-[#464646] text-lg font-semibold mb-4 px-5">
            National
          </h2>
          <div className="flex flex-col gap-3 mb-6 px-5">
            <MobileRankCard
              icon={
                <svg viewBox="0 0 512 512" className="w-10 h-10 text-[#016853]">
                  <path
          ...
          <div className="flex flex-col gap-3 mb-6 px-5">
            <MobileRankCard
              ...
              title="Best Private Colleges in Massachusetts"
              value="#2 of 58"
            ...
            <MobileRankCard
              ...
              title="Colleges with the Best Academics in Massachusetts"
              value="#3 of 62"
          ...
          <div className="flex flex-col gap-3 mb-6 px-5">
            <MobileRankCard
              ...
              title="Best Colleges in New Haven Area"
              value="#1 of 15"
...
export default RankingModalMobile;
