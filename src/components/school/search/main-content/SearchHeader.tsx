...
const SearchHeader: React.FC<SearchHeaderProps> = ({
  ...
  const filterCount = useMemo(() => {
    ...
    if (currentFilters.program) count += 1;
    if (currentFilters.majors) count += 1;
    if (
      Array.isArray(currentFilters.collegeTypeColleges) &&
      currentFilters.collegeTypeColleges.length
    ) {
      count += currentFilters.collegeTypeColleges.length;
    }
  ...
  const handleSearchSubmit = (e?: React.FormEvent) => {
    ...
    console.log("Searching for:", whatSearchQuery, "in", whereSearchQuery);
  ...
  const getUserLocation = () => {
    setWhereSearchQuery("Getting your location...");
          ...
          fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=10`)
            ...
            .then(data => {
              const city = data.address.city || 
                         ...
                         data.address.county || 
                         'Unknown location';
              setWhereSearchQuery(city);
            })
            .catch(error => {
              setWhereSearchQuery('Location not available');
              console.error('Error getting location name:', error);
            });
        },
        error => {
          if (error.code === error.PERMISSION_DENIED) {
            setWhereSearchQuery('Location access denied');
          } else {
            setWhereSearchQuery('Unable to get location');
          }
          console.error('Geolocation error:', error);
        }
      );
    } else {
      setWhereSearchQuery('Geolocation not supported');
    }
  };

  const handleMapToggle = () => {
    if (window.innerWidth < 768) {
      setIsMapDrawerOpen(!isMapDrawerOpen);
    } else {
      ...
      <div className="hidden md:flex items-center justify-between p-4 px-6 border-b border-[rgba(0,0,0,0.08)] bg-white">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`relative ${isSearchActive ? 'flex' : 'hidden'} items-center gap-3`}>
            ...
            <div className="flex items-center bg-white border border-[rgba(0,0,0,0.1)] rounded h-10 w-[420px] pr-[5px]">
              <div className="flex items-center flex-1 h-full">
                <div className="flex items-center flex-1 h-full">
                  <div className="text-sm font-semibold text-[#4A4A4A] px-[5px] whitespace-nowrap">What</div>
                  <div className="flex items-center relative flex-1 h-full">
                    <input
                      ...
                      value={whatSearchQuery}
                      onChange={(e) => setWhatSearchQuery(e.target.value)}
                      autoFocus
                    />
                    {whatSearchQuery && (
                      <button 
                        onClick={() => setWhatSearchQuery("")}
                        className="w-5 h-5 bg-none border-none flex items-center justify-center cursor-pointer text-[#5F5F5F] opacity-70 hover:opacity-100 flex-shrink-0"
                      >
                ...
                <div className="h-5 text-[rgba(0,0,0,0.2)] mx-1 text-base leading-5">|</div>
                
                <div className="flex items-center flex-1 h-full">
                  <div className="text-sm font-semibold text-[#4A4A4A] px-[5px] whitespace-nowrap">Where</div>
                  <div className="flex items-center relative flex-1 h-full">
                    <input
                      type="text"
                      className="border-none outline-none p-0 text-sm w-full h-full bg-transparent"
                      placeholder="Enter a city, state or country"
                      value={whereSearchQuery}
                      onChange={(e) => setWhereSearchQuery(e.target.value)}
                    />
                    {whereSearchQuery && (
                      <button 
                        onClick={() => setWhereSearchQuery("")}
                        className="w-5 h-5 bg-none border-none flex items-center justify-center cursor-pointer text-[#5F5F5F] opacity-70 hover:opacity-100 flex-shrink-0"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
                          <path d="M6.34314 4.22183C5.75736 3.63604 4.80761 3.63604 4.22182 4.22183C3.63604 4.80761 3.63604 5.75736 4.22182 6.34315L9.87868 12L4.22182 17.6569C3.63604 18.2426 3.63604 19.1924 4.22182 19.7782C4.80761 20.364 5.75736 20.364 6.34314 19.7782L12 14.1213L17.6569 19.7782C18.2426 20.364 19.1924 20.364 19.7782 19.7782C20.364 19.1924 20.364 18.2426 19.7782 17.6569L14.1213 12L19.7782 6.34315C20.364 5.75736 20.364 4.80761 19.7782 4.22183C19.1924 3.63604 18.2426 3.63604 17.6569 4.22183L12 9.87868L6.34314 4.22183Z" fill="currentColor"/>
                    ...
                    <button 
                      ...
                      className="w-6 h-6 bg-none border-none flex items-center justify-center cursor-pointer text-[#0093B0] opacity-80 hover:opacity-100 flex-shrink-0 -mr-3"
                    >
              ...
              <button 
                ...
                className="w-7 h-7 rounded-full bg-[#0093B0] border-none flex items-center justify-center cursor-pointer ml-2 flex-shrink-0"
              >
          ...
          {/* Expand/Collapse Button — expands layout by ~25% (1055 → 1318.75px) */}
          <button
            onClick={() => {
      ...
      <div className="md:hidden relative">
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-white shadow-sm border-b border-[rgba(0,0,0,0.08)]">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Expand/Collapse — expands layout by ~25% on mobile too */}
            <button
              onClick={() => {
                setIsContainerExpanded(!isContainerExpanded);
                onContainerExpandChange?.(!isContainerExpanded);
            ...
            >
              {isContainerExpanded ? (
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
                  <path d="M4 4m0 2a2 2 0 0 1 2 -2h12a2 2 0 0 1 2 2v12a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2z"></path>
                  <path d="M15 4v16"></path>
                  <path d="M9 10l2 2l-2 2"></path>
                </svg>
              ...
              <div className="flex items-center flex-1 bg-[#f5f5f7] border border-[rgba(0,0,0,0.1)] rounded-md px-3 h-10">
                ...
                {whereSearchQuery && (
                    <button
                        type="button"
                        onClick={() => setWhereSearchQuery("")}
                        className="w-5 h-5 flex items-center justify-center text-[#5F5F5F] opacity-70 hover:opacity-100 transition-opacity"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
                            <path d="M6.34314 4.22183C5.75736 3.63604 4.80761 3.63604 4.22182 4.22183C3.63604 4.80761 3.63604 5.75736 4.22182 6.34315L9.87868 12L4.22182 17.6569C3.63604 18.2426 3.63604 19.1924 4.22182 19.7782C4.80761 20.364 5.75736 20.364 6.34314 19.7782L12 14.1213L17.6569 19.7782C18.2426 20.364 19.1924 20.364 19.7782 19.7782C20.364 19.1924 20.364 18.2426 19.7782 17.6569L14.1213 12L19.7782 6.34315C20.364 5.75736 20.364 4.80761 19.7782 4.22183C19.1924 3.63604 18.2426 3.63604 17.6569 4.22183L12 9.87868L6.34314 4.22183Z" fill="currentColor"/>
                        </svg>
