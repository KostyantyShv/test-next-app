...
import { FC, useState, useLayoutEffect, useRef, useCallback, useEffect } from 'react';
...
const mockSources: Source[] = [
  {
    ...
    description: 'TikTok, the popular video-sharing platform owned by ByteDance, is making a significant shift in its content moderation strategy by laying off hundreds of human moderators and increasing its reliance on artificial intelligence (AI) for content review.',
    source: 'BT',
    ...
    subItems: [
      {
        ...
        description: 'Experts suggest that AI-driven content moderation could lead to more consistent enforcement of platform policies.'
      },
      {
        id: 2,
        title: 'Impact on Human Moderators',
        description: 'The shift towards AI moderation raises concerns about job security for human content moderators.'
      }
    ]
  },
  {
    id: 2,
    title: 'Rebrands as Halo Studios, working on multiple Unreal Engine 5 games',
    description: "Microsoft's gaming division has confirmed that the newly rebranded Halo Studios is actively developing several games using Unreal Engine 5, marking a new chapter in the Halo saga.",
    source: 'CNBC',
    ...
    subItems: [
      {
        ...
        description: 'The rebranding to Halo Studios signals a new direction and expanded scope for the development team.'
      },
      {
        id: 2,
        title: 'Unreal Engine 5 Benefits',
        description: 'The adoption of Unreal Engine 5 brings advanced graphics capabilities and improved development tools to the Halo franchise.'
      },
      {
        id: 3,
        title: 'Multiple Projects Scope',
        description: 'The studio is simultaneously working on several games, suggesting a broader expansion of the Halo universe.'
      }
    ]
  }
];
...
const mockBigIdeas: BigIdea[] = [
  {
    ...
    content: 'The beauty comes from connecting principles for high conversion rates with world-class design.',
    timestamp: 'Yesterday',
    hasHighlight: true,
    likes: 15
  },
...
const mockQA: QA[] = [
  ...
  },
  {
    ...
    answer: 'Machine learning enables computers to learn from data and improve their performance without being explicitly programmed. Unlike traditional programming where rules are manually coded, ML algorithms automatically discover patterns and make decisions based on examples.',
    votes: 89
  },
  {
    id: 3,
    question: 'What are the key principles of responsive web design?',
    answer: 'Responsive web design is built on three core principles: fluid grids that use relative units, flexible images that scale within their containers, and media queries that allow designs to adapt to different viewport sizes. This ensures websites provide optimal viewing experience across various devices.',
    votes: 73
  }
];
...
const mockSummary: Summary[] = [
  {
    id: 1,
    content: 'TikTok is transitioning from human moderators to AI-based content moderation systems.',
    type: 'key_point'
  },
  {
    id: 2,
    content: 'The shift raises concerns about job security and content quality control.',
    type: 'key_point'
  },
  {
    id: 3,
    content: 'AI moderation promises more consistent policy enforcement but faces challenges with context understanding.',
    type: 'conclusion'
  }
];
...
export const RightSidebar: FC = () => {
  const { isCollapsed, isExpanded, activeTab, setIsCollapsed, setIsExpanded, setActiveTab } = useSidebar();
  ...
  const checkScroll = useCallback(() => {
    if (tabsContainerRef.current) {
      ...
      setCanScrollRight(hasHorizontalScroll && container.scrollLeft < maxScroll - 1);
  ...
  const scrollToActiveTab = useCallback(() => {
    if (activeTabRef.current && tabsContainerRef.current) {
      ...
      if (tabLeft < containerLeft || tabRight > containerRight) {
        const scrollPosition = tabLeft - (container.clientWidth - tab.clientWidth) / 2;
        container.scrollTo({
          left: scrollPosition,
          behavior: 'smooth'
        });
    ...
    <>
      {((windowWidth < 1200 && !isCollapsed && isExpanded) || 
        ...
        <div 
          ...
          onClick={() => windowWidth < 1000 ? setIsCollapsed(true) : setIsExpanded(false)}
        />
      )}
        ...
        <div className="flex flex-col h-full">
          ...
          {isCollapsed ? (
            <div className="h-16 flex items-center justify-center border-b border-border">
              ...
              >
                <Icon name="expand" size="sm" className="text-gray-500 rotate-180" />
              </button>
            </div>
          ) : (
              ...
              <div className="border-b border-border relative">
                <div className="flex items-center">
                  {canScrollLeft && (
                    ...
                    >
                      <Icon name="chevron-left" size="sm" className="text-gray-500" />
                    </button>
                  )}
              ...
              <div className="flex-1 overflow-y-auto">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center justify-between gap-2 mt-2">
                                  ...
                                  <button
                                    ...
                                    aria-label={expandedSources.includes(source.id) ? "Collapse" : "Expand"}
                                  >
                                    <Icon 
                                      name="chevron-down" 
                                      size="sm" 
                                      className={cn(
                                        ...
                                        expandedSources.includes(source.id) && "transform rotate-180"
                                      )}
                                    />
                                  </button>
                                </div>
                ...
                {activeTab === 'bigIdeas' && (
                  <div className="flex flex-col gap-4 p-4">
                    {mockBigIdeas.map((idea) => (
                      <div key={idea.id} className="flex flex-col bg-gray-50 rounded-xl p-3">
                        <div className="flex flex-col gap-2 min-w-0 ml-2">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button className="text-xs text-gray-400">DELETE</button>
                            </div>
                          </div>
                          
                          <p className="text-sm text-gray-700 mt-0.5">{idea.title}</p>
                              ...
                              <div className="pl-3 py-2 bg-blue-50/50">
                                ...
                                <div className="flex justify-end mt-1">
                                  <div className="flex items-center gap-1 bg-gray-100/80 rounded-full px-2 py-0.5">
                                    <Icon name="pen" size="xs" className="text-gray-400" />
                                    <span className="text-xs text-gray-400">0:{idea.likes}</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                ...
                {activeTab === 'qa' && (
                  <div className="flex flex-col gap-2 p-4">
                    {mockQA.map((qa) => (
                      <div key={qa.id} className="flex flex-col bg-gray-50 rounded-xl">
                        <div className="flex gap-3 p-3">
                          <div className="flex-shrink-0 w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center">
                            <Icon name="question-chat" size="sm" className="text-blue-500" />
                          </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              >
                                <Icon 
                                  name={expandedQuestions.includes(qa.id) ? "minus" : "plus"} 
                                  size="sm" 
                                    ...
                                    <div className="flex justify-end mt-2">
                                      <div className="flex items-center gap-1 bg-white rounded-full px-3 py-1">
                                        <button className="hover:bg-gray-100 rounded p-0.5 transition-colors">
                                          <Icon name="arrow-up" size="sm" className="text-gray-400" />
                                        </button>
                                        <span className="text-sm text-gray-500 min-w-[2ch] text-center">
                                          {qa.votes}
                                        </span>
                                        <button className="hover:bg-gray-100 rounded p-0.5 transition-colors">
                                          <Icon name="arrow-down" size="sm" className="text-gray-400" />
                                        </button>
                                      </div>
                                    </div>
                                  )}
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <span className={cn(
                            ...
                            item.type === 'key_point' ? "bg-green-100 text-green-600" :
                            item.type === 'conclusion' ? "bg-blue-100 text-blue-600" :
                            "bg-gray-100 text-gray-600"
                          )}>
                            {item.id}
                          </span>
                          <p className="flex-1 text-sm text-gray-600">{item.content}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs text-gray-600">{item.relevance}% relevant</span>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              {actionButtons.map((button) => (
                ...
                >
                  <Icon 
                    ...
                    className={cn(
                      "shrink-0",
                      activeTab === button.tabId ? "text-blue-500" : "text-gray-500"
                    )}
                  />
